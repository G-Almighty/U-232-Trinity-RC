...
$added = TIME_NOW;
$secret = mksecret();
$hash2 = t_Hash($email, $wantusername, $added);
$hash3 = t_Hash($secret, $birthday, $email);


$wantpasshash = make_passhash($hash2, hash("ripemd160", $wantpassword), $hash3);
...
$ret = sql_query("INSERT INTO users (username, hash4, hash3, hash2, secret, editsecret, birthday, country, gender, pin_code, stylesheet, passhint, hintanswer, email, status, " . (!$arr[0] ? "class, " : "") . "added, last_access, time_offset, dst_in_use, free_switch) VALUES (" . implode(",", array_map("sqlesc", array(
    $wantusername,
    $wantpasshash,
	$hash3,
	$hash2,
...
$passh = h_cook($hash2, $_SERVER["REMOTE_ADDR"], $id);
/*=== for dupe account ===*/
...
if(is_null($logs['loginhash']) || $logs['loginhash'] != $hashlog){
