...
function userlogin()
{
    ...
    if (!$id or (strlen(get_mycookie('pass')) != 128) or (get_mycookie('hashv') != HashIt($id,
                get_mycookie('pass'))) or (get_mycookie('log_uid') != make_hash_log($id, get_mycookie('pass')))) {
        return;
    }
...
function set_mycookie($name, $value = "", $expires_in = 0, $sticky = 1)
{
    ...
    if (PHP_VERSION < 5.2) {
        ...
        @setcookie($name, $value, [
            'expires' => $expires,
            'path' => $TRINITY20['cookie_path'],
            'domain' => $TRINITY20['cookie_domain'],
            'secure' => false,
            'httponly' => true,
            'samesite' => 'Strict',
        ]);
...
function flood_limit($table)
{
    ...
    if (!file_exists($TRINITY20['flood_file']) || !is_array($max = unserialize(file_get_contents($TRINITY20['flood_file'])))) {
        return;
    }
